...
const DEFAULT_DATA_DIR = path.resolve(process.cwd(), "src/data/video-batch");
const DEFAULT_OUTPUT_DIR = path.resolve(process.cwd(), "outputs/video-batch");
const DEFAULT_CSV_FILE = "requests.csv";

const fileLocks = new Map<string, Promise<void>>();

export function getVideoBatchCsvPath(fileName?: string) {
  const safeName = sanitizeCsvFileName(fileName ?? DEFAULT_CSV_FILE);
  return path.join(DEFAULT_DATA_DIR, safeName);
}

export function getVideoBatchOutputDir() {
  return DEFAULT_OUTPUT_DIR;
}

export function ensureVideoBatchDirectories() {
  fs.mkdirSync(DEFAULT_DATA_DIR, { recursive: true });
  fs.mkdirSync(DEFAULT_OUTPUT_DIR, { recursive: true });
}

export function readVideoBatchCsv(filePath: string): CsvData {
...
export function writeVideoBatchCsv(filePath: string, header: string[], rows: CsvRow[]) {
