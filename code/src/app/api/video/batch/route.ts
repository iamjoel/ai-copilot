...
import {
  ensureHeaderColumns,
  ensureVideoBatchDirectories,
  getVideoBatchCsvPath,
  getVideoBatchOutputDir,
  readVideoBatchCsv,
  writeVideoBatchCsv,
  withCsvLock,
  type CsvRow,
...
export async function POST(req: Request) {
  try {
    ...
    const csvPath = getVideoBatchCsvPath(inputFile);
    ...
    if (error instanceof BatchInputError) {
      logger.error({ error }, "Video batch input error");
      return Response.json({ error: error.message }, { status: 400 });
    }
    logger.error({ error }, "Video batch error");
    return Response.json(
      { error: "Unable to run video batch right now." },
      { status: 500 },
    );
  }
}
...
async function runBatch(csvPath: string, limit: number | null): Promise<BatchResult> {
  ...
  const { header, rows } = readVideoBatchCsv(csvPath);
  if (!header.length) {
  ...
  for (let index = 0; index < runCount; index += 1) {
    ...
    if (!input) {
      ...
      processed += 1;
      writeVideoBatchCsv(csvPath, header, rows);
      continue;
    }

    try {
      const outputPath = buildOutputPath(index + 1);
      await generateVideo(input, outputPath);
      updateRow(row, {
...
async function generateVideo(prompt: string, outputPath: string) {
  ...
  while (!operation.done) {
    logger.info({ result: "waiting for video generation" });
  ...
  if (!video) {
    throw new Error("No video returned by provider.");
  }
...
function buildOutputPath(rowIndex: number) {
  const outputDir = getVideoBatchOutputDir();
  const timestamp = Date.now();
  const filename = `video-${rowIndex}-${timestamp}.mp4`;
